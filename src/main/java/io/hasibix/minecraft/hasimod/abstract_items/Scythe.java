package io.hasibix.minecraft.hasimod.abstract_items;
...
public abstract class Scythe extends SwordItem {
	private List<StatusEffectInstance> effects = new ArrayList<>();
	private int bladesAmount = 10;

	public List<StatusEffectInstance> getEffects() {
		return effects;
	}

	public Scythe(ToolMaterial toolMaterial, int attackDamage, float attackSpeed, Settings settings, int bladesAmount) {
	...
	public TypedActionResult<ItemStack> use(World world, PlayerEntity entity, Hand hand) {
		if (entity instanceof PlayerEntity) {
			...
			if (!world.isClient) {
				for (int i = 0; i < this.bladesAmount; i++) {
					float value = -1F + (i * (2F / this.bladesAmount));
			...
			user.damage(new io.hasibix.minecraft.hasimod.damage_sources.Scythe(), 2F);
